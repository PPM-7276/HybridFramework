package com.mindbowser.reports;
...
public class ExtentRep implements IReporter {
			...
			for (ITestResult result : tests.getAllResults()) {
				...
				if (result.getThrowable() != null) {
					...
					test.log(LogStatus.FAIL, "TEST CASE FAILED IS " + result.getThrowable());// to Add error/exception
																								// in extent report
					String screenshotPath = screenshotFile.getScreenShotPath(SpicejetPage.driver, result.getName());
					test.log(LogStatus.FAIL, test.addScreenCapture(screenshotPath));// to add screenshot in extent
																					// report
					// test.log(LogStatus.FAIL, test.addScreenCapture(screenshotPath));// to add
					// screencase/video in extent report
				} else {
